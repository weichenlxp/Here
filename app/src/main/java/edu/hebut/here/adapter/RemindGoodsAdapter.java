...
public class RemindGoodsAdapter extends RecyclerView.Adapter<RemindGoodsAdapter.MyViewHolder> {
    ...
    public RemindGoodsAdapter(Context mContext, OverdueActivity activity, String isOverdue) {
        ...
        if (isOverdue.equals("1")) {//过期
            goodsCursor = queryGoods(this.mContext, new String[]{"goodsID", "goodsName", "goodsPhoto1"}, "userID=? AND isOvertime=?", new String[]{String.valueOf(userID), "1"});
            overdue = mContext.getResources().getIdentifier("ic_overdue", "drawable", mContext.getPackageName());
        } else {//临期
            goodsCursor = queryGoods(this.mContext, new String[]{"goodsID", "goodsName", "goodsPhoto1"}, "userID=? AND isCloseOvertime=? AND isOvertime=?", new String[]{String.valueOf(userID), "1", "0"});
